...
/*#define R 0//Rho
#define D 1//Minimum Distance to neighbour with bigger rho
#define N 2//Nearest neighbour with bigger rho
#define C 3//Cluster number - default 0
#define BD 4//distance to nearest point from other cluster if iB
#define iB 5//is Border Region*/
...
void fix(Matrix<float> mat, Matrix<float> output, int rows, int cols, int maxR)
{
	std::vector<int> v;
	for (int i = 0; i < rows; ++i)
		if (output[i][N] == INT_MAX && std::find(v.begin(), v.end(), output[i][R]) != v.end())
			v.push_back(output[i][R]++);
	if (v.size() > 2)
	{
		cout << "Again" << endl;
...
void distanceToOtherCluster(Matrix<float> mat, Matrix<float> output, int rows, int cols, int maxR, float radius)
{
	...
	for (int i = 0; i < rows; ++i)
	{
		...
		int nn = index.radiusSearch(q, indices, dists, radius, flann::SearchParams(128));
		int ix = output[i][C];
		output[i][BD] = std::numeric_limits<float>::max();
		output[i][iB] = 0;
		for (int k = 0; k < nn; ++k)
		{
...
std::vector<clusterInfo> getMaxBorderDenesity(Matrix<float> output, int numberOfClusters, int rows, float radius)
{
	...
	for (int r = 0; r < rows; ++r)
	{
		...
		{
			if (output[r][C] == i && dens[i] < output[r][R] && output[r][4] < radius && output[r][iB])
			{
				dens[i] = output[r][R];
				break;
			}
...
void excludeHaloPoints(Matrix<float> output, std::vector<clusterInfo> cInfo, int numberOfClusters, int rows)
{
	...
	for (int r = 0; r < rows; ++r)
	{
		for (int i = 1; i <= numberOfClusters; ++i)
		{
			if (output[r][C] == i && output[r][R] < cInfo[i - 1].maxDenesity / 2)
				output[r][C] = 0;
		}
	}
}
...
int main(int argc, char **argv)
{
	...
	std::vector<clusterInfo> ci = getMaxBorderDenesity(extraData, numOfClusters, rows, radius);
	for (int i = 0; i < numOfClusters; ++i)
		std::cout << ci[i].clusterIndex << " " << ci[i].maxDenesity << std::endl;
	//excludeHaloPoints(extraData, ci, numOfClusters, rows);
